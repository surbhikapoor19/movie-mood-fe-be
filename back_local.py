"""
back_local.py  —  FastAPI backend (locally executed product), port 9010
Uses llama-cpp-python with a small GGUF model — no external API calls for inference.
"""
...
# ============================================================================
# MODEL INITIALIZATION — llama-cpp-python with GGUF
# ============================================================================
print(f"[MODE] Local backend — loading GGUF model: {GGUF_REPO}/{GGUF_FILENAME}")

from llama_cpp import Llama
from huggingface_hub import hf_hub_download

# Download GGUF model if not already cached
print(f"[INFO] Downloading/verifying GGUF model...")
...
llm = Llama(
    ...
    n_ctx=2048,       # Context window — enough for few-shot prompts
    n_threads=2,      # Conservative for shared VM
    verbose=False,
)
print(f"[MODE] Local GGUF model loaded successfully!")
...
# ============================================================================
# LOCAL MODEL INFERENCE — llama-cpp-python
# ============================================================================
def run_local_model(messages: list[dict], max_tokens: int, temperature: float) -> str:
    """Run inference using the local GGUF model via llama-cpp-python."""
    try:
        print(f"[INFO] Starting local GGUF inference...")
...
# FASTAPI
# ============================================================================
app = FastAPI(title="Movie Recommendation Backend (Local Mode)")
...
def chat_endpoint(req: ChatRequest):
    ...
    print(f"\n{'='*60}")
    print(f"[INFO] New chat request (LOCAL MODE)")
